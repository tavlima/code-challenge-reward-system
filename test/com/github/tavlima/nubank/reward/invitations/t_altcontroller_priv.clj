(ns com.github.tavlima.nubank.reward.invitations.t-altcontroller-priv
  (:use clojure.test
        midje.sweet
        [midje.util :only [expose-testables]]
        com.github.tavlima.nubank.reward.invitations.altcontroller)
  (:require [com.github.tavlima.nubank.reward.tree.location :as z]
            [com.github.tavlima.nubank.reward.tree.domain :as d]))

(expose-testables com.github.tavlima.nubank.reward.invitations.altcontroller)

(defn- test-user
  ([] (d/create-user :default))
  ([ks] (merge (d/create-user :default) ks)))

(defn- new-tree [root uids]
  (merge (d/create-tree) {:root root :uids uids}))
...
(facts "about `add-user`"
       (let [tree (new-tree (-> (d/create-user 1)
                                (d/verify!)
         ...
         (fact "it locates the inviter node, adds the new invitee node as it's child, adds the invitee id to the tree user's hashset and proceed with the verification process"
               ...
               (add-user tree 2 4) => (-> tree
                                          (assoc-in [:root :invited 0 :invited 1] (d/create-user 4)) ; adds (4) to (2)
               ...
               (add-user tree 3 4) => (-> tree
                                          (assoc-in [:root :invited 0 :invited 0 :verified] true) ; marks (3) as verified
                                          (assoc-in [:root :invited 0 :invited 0 :invited] [(d/create-user 4)]) ; adds (4) to (3)
                                          (update-in [:root :invited 0 :score] #(+ % 1)) ; adds +1 to (2) score
                                          (update-in [:root :score] #(+ % 1/2)) ; adds +1/2 to (1) score
                                          (update :uids #(conj % 4)))) ; adds 4 to the tree hashset

         (fact "it works even on empty trees"
                 (add-user (d/create-tree) 1 2) => (-> (d/create-tree)
                                                       (assoc :root (-> (d/create-user 1)
                                                                        (d/verify!)
                                                                        (d/addInvited (d/create-user 2))))
                                                       (assoc :uids #{1 2})))
