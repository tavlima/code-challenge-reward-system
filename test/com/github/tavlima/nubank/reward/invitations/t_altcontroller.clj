(ns com.github.tavlima.nubank.reward.invitations.t-altcontroller
  (:use clojure.test
        midje.sweet
...
(facts "about `create-tree`"
       (fact "it creates an empty tree"
             (create-tree) => (d/create-tree)))

(facts "about `get-user`"
       (let [user3 (d/create-user 3)
             user2 (-> (d/create-user 2)
                       (d/addInvited user3))
             user1 (-> (d/create-user 1)
                       (d/addInvited user2))
...
(facts "about `invite`"
       (let [emptyTree (d/create-tree)
             tree12 (add-user emptyTree 1 2)
             tree123 (add-user tree12 2 3)]
         ...
         (fact "it populates an empty tree with the inviter user, if the tree is empty"
               ...
               (invite (d/create-tree) 1 2) => tree12)))

(facts "about `ranking`"
       (let [tree (-> (d/create-tree)
                      (add-user 1 2)
                      (add-user 1 3)
                      (add-user 3 4)
                      (add-user 2 4)
